...
full_date_format_dotted = '%d.%m.%Y'


class EmploymentCustom(models.Model):
    _inherit = 'hr.employee'

    def read_strata_csv_file(self):
                        ...
                        if line_count == 0:
                            ...
                            if log_type == 'Success':
                                ...
                                event_reason = self.env['sap.event.type.reason'].search(
                                    [('name', '=', reason_code), ('event_type_id', '=', event_type.id)])

                                event = self.env['hr.employee.event'].search(
                                    [('employee_strata_id', '=', object_id), ('name', '=', event_type.id),
                                     ('is_processed', '=', False),
                            ...
                            if log_type == 'Success':
                                ...
                                for ev in event:
                                    if start_date == ev.from_date.strftime(
                                            full_date_format_dotted) and employee_id == ev.related_event.employee_id.strata_id:
