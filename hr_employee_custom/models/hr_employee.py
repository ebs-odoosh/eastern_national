...
class EmployeeEvent(models.Model):
    ...
    employee_strata_id = fields.Char(string='Strata ID', required=False)
    ...
    def onchange_employee(self):
        ...
        strata_id = employee.strata_id

        real_employee = self.env['hr.employee'].search([('strata_id', '=', strata_id)], limit=1)
        ...
        if len(events) == 0:
            self.employee_strata_id = employee.strata_id if employee.strata_id else ''
            ...
            self.employee_strata_id = latest_event.employee_strata_id if latest_event.employee_strata_id else ''
    ...
    def do_changes(self, event):
        ...
        # check strata id
        is_changed_strata_id = is_changed(employee.strata_id, event.employee_strata_id)
        if is_changed_strata_id:
            employee.strata_id = event.employee_strata_id
        # check org
...
class EventCompensation(models.Model):
    ...
    #     if employee.id:
    #         vals['employee_strata_id'] = employee.strata_id if employee.strata_id else ''
    #         vals['org_unit'] = employee.department_id.code if employee.department_id.code else ''
    #         vals['line_manager_id'] = employee.parent_id.strata_id if employee.parent_id.strata_id else ''
