...
class ApprovalRequest(models.Model):
    ...
    def get_related_request_domain(self):
        if self.env.user.has_group('security_rules.group_strata_employee'):
    ...
    related_strata_id = fields.Char(related='related_employee_id.strata_id')
    ...
    employee_strata_id = fields.Char('Employee Strata ID')
    ...
    def get_read_only_approval(self):
        for rec in self:
            if self.env.user.has_group('security_rules.group_hc_employee'):
                ...
                rec.read_only_approval = False
            elif rec.request_status != 'new' and self.env.user.id == rec.approver_ids.filtered(
                    lambda x: x.status == 'pending').user_id.id:
                rec.read_only_approval = False
            else:
        ...
        if view_type == "form":
            ...
            for field in res['fields']:
                for node in doc.xpath("//field[@name='%s']" % field):
                    print(field)
                    if field == 'approver_ids' and self.env.user.has_group('security_rules.group_strata_employee'):
                        print('in fct')
                        node.set("readonly", "1")
    ...
    def _get_user_group(self):
        ...
        for rec in self:
            ...
            if user.has_group('security_rules.group_strata_employee'):
    ...
    def onchange_employee_transferred(self):
        ...
        if self.employee_transferred and self.category_id.is_internal_transfer_request == 'yes':
            self.employee_strata_id = self.employee_transferred.strata_id
    ...
    def create(self, vals):
        ...
        elif res.is_requisition_request == 'yes' and self.env.user.has_group('security_rules.group_strata_employee'):
            group = res.group
    ...
    def write(self, vals):
        ...
        if vals.get(
            ...
            self.section = self.related_employee_id.contract_id.section
            self.subsection = self.related_employee_id.contract_id.subsection
        elif self.is_requisition_request == 'yes' and self.env.user.has_group('security_rules.group_strata_employee'):
        ...
        for request in self:
            if request.is_requisition_request == 'yes':
                ...
                if request.employee_transferred:
                    request.name = request.category_id.name + " - " + request.employee_transferred.name + " - " + request.employee_transferred.strata_id
                    continue
    ...
    #             # 'employee_strata_id': event_type.id,
    ...
    has_employee_strata_id = fields.Selection(related="category_id.has_employee_strata_id")
